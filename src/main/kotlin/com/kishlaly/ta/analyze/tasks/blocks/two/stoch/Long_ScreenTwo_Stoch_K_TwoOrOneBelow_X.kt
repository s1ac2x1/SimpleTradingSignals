package com.kishlaly.ta.analyze.tasks.blocks.two.stoch

import com.kishlaly.ta.analyze.tasks.ThreeDisplays.Config.STOCH_CUSTOM
import com.kishlaly.ta.analyze.tasks.blocks.two.ScreenTwoBlock
import com.kishlaly.ta.model.BlockResult
import com.kishlaly.ta.model.BlockResultCode
import com.kishlaly.ta.model.SymbolData
import com.kishlaly.ta.model.indicators.Stochastic
import com.kishlaly.ta.utils.Log
import com.kishlaly.ta.utils.SymbolDataUtils

/**
 * one of the two stochastic %K values is less than 20
 */
class Long_ScreenTwo_Stoch_K_TwoOrOneBelow_X : ScreenTwoBlock {
    override fun check(screen: SymbolData): BlockResult {
        if (STOCH_CUSTOM < 0) {
            throw RuntimeException("ThreeDisplays.Config.STOCH_CUSTOM not set")
        }
        val stoch = SymbolDataUtils(screen, Stochastic::class.java)
        val oneBelowExtraLow = stoch[2].slowK < STOCH_CUSTOM || stoch[1].slowK < STOCH_CUSTOM

        if (!oneBelowExtraLow) {
            Log.recordCode(BlockResultCode.STOCH_K_NOT_EXTRA_OVERSOLD_SCREEN_2, screen)
            Log.addDebugLine("One of the last two stochastic %Ks is at least 20 on the second screen")
            return BlockResult(screen.lastQuote, BlockResultCode.STOCH_K_NOT_EXTRA_OVERSOLD_SCREEN_2)
        }

        return BlockResult(screen.lastQuote, BlockResultCode.OK)
    }
}