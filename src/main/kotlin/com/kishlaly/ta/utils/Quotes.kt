...
class Quotes {
    ...
    companion object {
        ...
        fun dayToWeek(dailyQuotes: List<Quote>): List<Quote> {
            ...
            var weekSorted: List<Quote>
            for (i in 0 until dailyQuotes.size - 1) {
                try {
                    ...
                    val currentQuoteDate =
                        Dates.getTimeInExchangeZone(currentQuote.timestamp, exchangeTimezome)
                    ...
                    if (nextQuoteDate.dayOfWeek == currentQuotePlusOneDayDate.dayOfWeek) {
                        ...
                        weekSorted = week.sortedBy { it.timestamp }.toMutableList()
...
fun main() {
    val kotlin = CacheReader.loadQuotesFromDiskCache("AAPL")
    val kotlinWeekly = Quotes.dayToWeek(kotlin)
    println(kotlinWeekly.size)
}