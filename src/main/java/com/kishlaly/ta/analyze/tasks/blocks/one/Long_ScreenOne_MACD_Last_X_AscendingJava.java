 ...
 */
public class Long_ScreenOne_MACD_Last_X_AscendingJava implements ScreenOneBlockJava {
    @Override
    public BlockResultJava check(SymbolDataJava screen) {
        ...
        int count = 0;
        for (int i = screen_1_MACD.size() - ThreeDisplaysJava.Config.EMA26_TOTAL_BARS_CHECK; i < screen_1_MACD.size() - 1; i++) {
        ...
        if (count < ThreeDisplaysJava.Config.EMA26_TOTAL_BARS_CHECK) {
            LogJava.recordCode(X_HISTOGRAMS_NOT_ASCENDING_SCREEN_1, screen);
            LogJava.addDebugLine(ThreeDisplaysJava.Config.EMA26_TOTAL_BARS_CHECK + " histograms do not grow on the long-term screen");
