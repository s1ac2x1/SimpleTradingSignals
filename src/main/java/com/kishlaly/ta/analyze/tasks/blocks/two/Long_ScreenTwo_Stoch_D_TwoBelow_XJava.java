 ...
 */
public class Long_ScreenTwo_Stoch_D_TwoBelow_XJava implements TaskBlockJava {
    @Override
    public BlockResultJava check(SymbolDataJava screen) {
        ...
        boolean bothBelowExtraLow = stoch2.getSlowD() < ThreeDisplaysJava.Config.STOCH_CUSTOM && stoch1.getSlowD() < ThreeDisplaysJava.Config.STOCH_CUSTOM;

        if (!bothBelowExtraLow) {
            LogJava.recordCode(STOCH_D_NOT_EXTRA_OVERSOLD_SCREEN_2, screen);
            LogJava.addDebugLine("Both last %D stochastics are at least " + ThreeDisplaysJava.Config.STOCH_CUSTOM + " on the second screen");
