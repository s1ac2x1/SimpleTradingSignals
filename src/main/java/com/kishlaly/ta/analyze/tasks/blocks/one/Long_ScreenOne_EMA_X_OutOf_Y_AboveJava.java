 ...
 */
public class Long_ScreenOne_EMA_X_OutOf_Y_AboveJava implements ScreenOneBlockJava {
    @Override
    public BlockResultJava check(SymbolDataJava screen) {
        if (ThreeDisplaysJava.Config.EMA26_TOTAL_BARS_CHECK < 0) {
            throw new RuntimeException("ThreeDisplays.Config.EMA26_TOTAL_BARS_CHECK not set");
        }
        if (ThreeDisplaysJava.Config.EMA26_ABOVE_BARS < 0) {
        ...
        int aboveEMA26 = 0;
        for (int i = screen_1_EMA26.size() - ThreeDisplaysJava.Config.EMA26_TOTAL_BARS_CHECK; i < screen_1_EMA26.size(); i++) {
            if (QuotesJava.isQuoteAboveEMA(screen.quotes.get(i), screen_1_EMA26.get(i).getValue())) {
                aboveEMA26++;
            }
        }
        if (aboveEMA26 < ThreeDisplaysJava.Config.EMA26_ABOVE_BARS) {
            LogJava.recordCode(NOT_ALL_NEEDED_QUOTES_ABOUT_EMA_SCREEN_1, screen);
            LogJava.addDebugLine("Out of the last " + ThreeDisplaysJava.Config.EMA26_TOTAL_BARS_CHECK + " quotes " + ThreeDisplaysJava.Config.EMA26_ABOVE_BARS + " are not above EMA26 on the long term screen");
